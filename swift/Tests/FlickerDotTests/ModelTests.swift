...
@Suite struct ModelTests {
    ...
    @Test func geometry7x7() {
        ...
        #expect(dots.count == Flicker.dotsFull)
        ...
        #expect(computeModel(allOff(1)).viewBox == Flicker.viewBoxFull)
    }

    @Test func geometry5x5() {
        let model = computeModel(allOff(1), small: true)
        let dots = model.dots
        #expect(dots.count == Flicker.dotsSmall)
        #expect(model.viewBox == Flicker.viewBoxSmall)
    ...
    @Test func kinds() {
        ...
        #expect(single.filter { $0.kind == .off }.count == Flicker.dotsFull - 1)
    ...
    @Test func smallModelReadsTheSafeArea() {
        ...
        for (i, dot) in small.dots.enumerated() {
            #expect(dot.pattern == grids.map { $0[Flicker.safeAreaIndices[i]] })
        }
    }

    @Test func smallFrame() {
        let frame = (0..<Flicker.dotsFull).map { $0 % 2 == 0 }
        let small = FlickerDot.toSmallFrame(frame)
        #expect(small.count == 25)
        #expect(small == Flicker.safeAreaIndices.map { frame[$0] })
    }

    @Test func safeAreaIndices() {
        #expect(Flicker.safeAreaIndices.count == 25)
        #expect(Flicker.safeAreaIndices.min() == 8)
        #expect(Flicker.safeAreaIndices.max() == 40)
    }
